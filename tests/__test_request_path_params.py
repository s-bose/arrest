...
async def test_request_path_params(
    ...
    service.add_resource(
        Resource(
            ...
            handlers=[
                ...
                (
                    Methods.GET,
                    "/{user_id:int}/profile/{id:int}/badges/{badge_id:uuid}",
                ),
            ],
        )
    )
    ...
    # if kwargs:
    #     await service.user.request(path=route, method=method, **kwargs)
    # else:
    #     await service.user.request(path=route, method=method)

    await service.user.request(method="GET", path="/profile", id=123)
